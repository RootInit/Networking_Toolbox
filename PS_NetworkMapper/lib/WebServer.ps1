...
function Invoke-ConnectAction {
    ...
    # Distinct from "outside the scopes": an empty list refuses everything, and "outside ()" reads
    # like a bug rather than the unconfigured state it is.
    if ($AllowedScopes.Count -eq 0) {
        Send-WebJson -Response $Response -StatusCode 400 -Object @{ error = "No crawl scope configured - set 'Allowed Scopes' in the Settings tab and save, then try again." }
        return
    }
    if (-not (Test-IpInAllowedScopes -IP $TargetIP -AllowedScopes $AllowedScopes)) {
        Send-WebJson -Response $Response -StatusCode 400 -Object @{ error = "IP is outside the configured allowed scopes ($($AllowedScopes -join ', '))" }
        return
    }
...
function Invoke-SaveConfigAction {
    ...
    # The crawl fence lives here rather than on the command line, so it has to be validated here too.
    # Not an injection vector - Test-IpInAllowedScopes only does string comparison - but a typo'd
    # entry silently fences nothing, which is the failure worth catching loudly.
    $NewScopes = $null
    if ($null -ne $Parsed.settings -and $null -ne $Parsed.settings.allowedScopes) {
        $NewScopes = @($Parsed.settings.allowedScopes |
            Where-Object { -not [string]::IsNullOrWhiteSpace($_) } |
            ForEach-Object { ([string]$_).Trim() })
        foreach ($Scope in $NewScopes) {
            if ($Scope -notmatch '^\d{1,3}(\.\d{1,3}){0,3}\.?\z') {
                Send-WebJson -Response $Response -StatusCode 400 -Object @{ error = "Invalid allowed scope '$Scope': expected a dotted IPv4 prefix such as 192.0.2. or 10.4.1.7" }
                return
            }
            foreach ($Octet in ($Scope.TrimEnd('.') -split '\.')) {
                if ([int]$Octet -gt 255) {
                    Send-WebJson -Response $Response -StatusCode 400 -Object @{ error = "Invalid allowed scope '$Scope': '$Octet' is not a valid octet (0-255)" }
                    return
                }
            }
        }
    ...
    try {
        ...
        # Same ordering, same reason. Applying it live is the point: a first-time operator sets the
        # scope in Settings and can scan immediately instead of restarting the server.
        if ($null -ne $NewScopes) {
            $script:AllowedScopes = $NewScopes
        }
...
function Start-MapperWebServer {
    param(
        ...
        # Empty is legal and fail-closed: nothing is crawlable until a scope is saved in Settings.
    ...
    $script:AllowedScopes = $AllowedScopes
            ...
            try {
                if ($Request.HttpMethod -eq "POST" -and $Request.Url.AbsolutePath -eq "/api/connect") {
                    if (-not (Test-SameOriginRequest -Request $Request -Port $Port)) {
                        ...
                        Invoke-ConnectAction -Response $Response -Body $Body -ConnectScriptPath $ConnectScriptPath -JunosUsername $script:JunosUsername -JunosPassword $script:JunosPassword -PowerShellExePath $PowerShellExePath -AllowedScopes $script:AllowedScopes
                    }
                    ...
                    if (-not (Test-SameOriginRequest -Request $Request -Port $Port)) {
                        ...
                        $Body = Read-WebRequestBody -Request $Request
                        Invoke-RescanAction -Response $Response -Body $Body -WorkerPath $WorkerPath -JunosUsername $script:JunosUsername -JunosPassword $script:JunosPassword -AllowedScopes $script:AllowedScopes
                    }
                    ...
                    if (-not (Test-SameOriginRequest -Request $Request -Port $Port)) {
                        ...
                        $Body = Read-WebRequestBody -Request $Request
                        Invoke-ScanNetworkAction -Response $Response -Body $Body -WorkerPath $WorkerPath -JunosUsername $script:JunosUsername -JunosPassword $script:JunosPassword -MaxConcurrent $MaxConcurrent -AllowedScopes $script:AllowedScopes -SnapshotDir $SnapshotDir -EncKey $EncKey -MacKey $MacKey -Salt $Salt -Iterations $Iterations
                    }
