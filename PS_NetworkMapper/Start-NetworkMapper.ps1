...
$JunosUsername = $null
$JunosPassword = $null
# The crawl fence, configured in the viewer's Settings tab rather than on the command line so no
# network prefix is baked into the repo. Empty is the safe state: Test-IpInAllowedScopes matches
# nothing against an empty list, so every crawl, rescan and connect is refused until one is set.
$AllowedScopes = @()
...
if ($NoEncryption) {
    if (Test-Path $ConfigPath) {
        try {
            ...
            # Tolerates the key being absent, a bare string, or an array with blank entries.
            $AllowedScopes = @(if ($ConfigParsed.settings) { $ConfigParsed.settings.allowedScopes |
...
if ($AllowedScopes.Count -eq 0) {
    Write-Host "No crawl scope configured - the crawl has no boundary to stay inside, so it will not start. Run .\Start-NetworkMapper.ps1 with no -SwitchIP, set 'Allowed Scopes' in the viewer's Settings tab, save, then re-run with -SwitchIP." -ForegroundColor Red
    exit 1
}
if (-not (Test-IpInAllowedScopes -IP $SwitchIP -AllowedScopes $AllowedScopes)) {
    Write-Host "SwitchIP '$SwitchIP' is outside the configured allowed scopes ($($AllowedScopes -join ', ')). Adjust them in the viewer's Settings tab if this IP should be permitted." -ForegroundColor Red
    exit 1
}
