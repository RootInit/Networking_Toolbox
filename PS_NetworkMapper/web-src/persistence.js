...
// Deliberately outside DEFAULT_SETTINGS: that map is all-numeric (the save loop parseFloats every
// key) and has no safe default here - an empty fence refuses everything, which is the intended
// unconfigured state, and a guessed prefix would silently fence the wrong network.
...
window.populateSettingsInputs = function() {
    ...
    var scopesEl = document.getElementById('setting-allowedScopes');
    if (scopesEl) scopesEl.value = (settings.allowedScopes || []).join('\n');
...
window.saveSettingsPanel = async function() {
    ...
    // Not DEFAULT_SETTINGS keys, but written here regardless: setLoadedSettings REPLACES the settings
    // object, so anything left out would be dropped on the next threshold save.
    var darkEl = document.getElementById('setting-darkMode');
    if (darkEl) settings.darkMode = darkEl.checked;

    var scopesEl = document.getElementById('setting-allowedScopes');
    // The server re-validates; this catches the typo before a round trip and names the bad entry.
    if (scopesEl) {
        var scopes = parseScopeList(scopesEl.value);
        var badScope = scopes.find(function (s) {
            if (!/^\d{1,3}(\.\d{1,3}){0,3}\.?$/.test(s)) return true;
            return s.replace(/\.$/, '').split('.').some(function (o) { return parseInt(o, 10) > 255; });
        });
        if (badScope) {
            window.setStatus("Settings not saved - '" + badScope + "' is not a valid IPv4 prefix (e.g. 192.0.2. or 10.4.1.7).", "red");
            return;
        }
        settings.allowedScopes = scopes;
    } else {
        // Field absent (older bundle): preserve whatever is already stored rather than clearing it.
        var priorSettings = window.getLoadedSettings ? window.getLoadedSettings() : {};
        if (priorSettings && priorSettings.allowedScopes) settings.allowedScopes = priorSettings.allowedScopes;
    }
