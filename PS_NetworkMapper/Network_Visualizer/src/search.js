...
// Shared renderer for #searchResults - used both by a text search (above) and by a
// dashboard stat drill-down (see window.drillDownStat in dashboard.js), so a drill-down
// list looks and behaves exactly like a search result list (same markup, same
// click-through). `opts` - `headerText`: shown in a sticky bar with a "Clear" link
// (drill-down only, since a text search's own input box already shows what's being
// searched for); `emptyText`: shown when `rows` is empty.
